...
class LinearQuadraticMFG():
    ...
    def __init__(self, time_domain, **kwargs):
        ...
        self.arg_list = ["A", "barA", "B","C", "Q", "barQ","S", "QT","barQT","ST","x0","sigma0", "nu"]
        self.A, self.barA, self.B, self.C, self.Q, self.barQ, self.S, self.QT, self.barQT, self.ST, self.x0, self.sigma0, self.nu  =  (kwargs[name] for name in self.arg_list)
    ...
    def _forward_dot_z(self,z,t):
        """Differential for z
        """
        dot_z = (self.A + self.barA - (self.B**2/self.C)*self.pfunc(t))*z - (self.B**2/self.C)*self.rfunc(t)
        return dot_z
    ...
    def _backward_dot_r(self,r,t):
        """Differential for r
        """
        dot_r = -( (self.A - (self.B**2/self.C)*self.pfunc(t))*r +\
    ...
    def _backward_dot_s(self, s, t):
        """Differential for s.
        """
        dot_s = -(self.nu*self.pfunc(t) - 0.5*(self.B**2/self.C)*self.rfunc(t) +\
                  self.rfunc(t)*self.barA*self.zfunc(t) + 0.5*(self.S**2*self.barQ)*self.zfunc(t)**2)
    ...
    def solve_nash_equilibrium(self, initial_z, initial_r, **kwargs):
        ...
        self.zfunc, self.rfunc = (self._create_time_function(v) for v in [self.z, self.r])
