...
class Filtration():
    ...
    def generate_paths(self):
        ...
        self.brownian_increments = torch.randn(size = (self.number_of_paths, len(self.time_domain)-1, self.spatial_dimensions)) * dt**0.5
        self.brownian_increments = torch.cat([torch.zeros(size = (self.number_of_paths, 1, self.spatial_dimensions)) , self.brownian_increments], dim = 1)
...
class BackwardSDE():
    ...
    def initialize_approximator(self, nn_args:dict = {}):
        self.y_approximator = FunctionApproximator(domain_dimension=self.spatial_dimensions + 1, output_dimension=1, **nn_args)
    ...
    def set_optimization_target(self, terminal_condition):
        optimization_target =  terminal_condition.repeat(repeats = (1,1, len(self.time_domain)))
        optimization_target = optimization_target.reshape((len(self.time_domain),self.filtration.number_of_paths, self.spatial_dimensions))
    ...
    def solve(self, approximator_args:dict = None):
        ...
        self.y_approximator.minimize_over_sample(self.filtration.brownian_paths, optimization_target, **approximator_args)
