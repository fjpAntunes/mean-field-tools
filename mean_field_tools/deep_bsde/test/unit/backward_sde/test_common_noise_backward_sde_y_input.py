...
def test_approximator_input_width():
    """Input is (t, W_t) over the padded time axis."""
    bsde = setup()

    assert bsde.set_approximator_input().shape == (
        NUMBER_OF_PATHS,
        NUMBER_OF_TIMESTEPS + bsde.padding_size,
        1 + SPATIAL_DIMENSIONS,
    )


def test_approximator_input_width_with_parameter():
    """A parameter is prepended to (t, W_t)."""
    bsde = setup(number_of_parameters=NUMBER_OF_PARAMETERS)

    assert bsde.set_approximator_input().shape == (
        NUMBER_OF_PATHS,
        NUMBER_OF_TIMESTEPS + bsde.padding_size,
        NUMBER_OF_PARAMETERS + 1 + SPATIAL_DIMENSIONS,
    )


def test_y_approximator_accepts_approximator_input():
    """The y approximator must be sized for what `set_approximator_input` builds."""
    for number_of_parameters in [0, NUMBER_OF_PARAMETERS]:
        bsde = setup(number_of_parameters)
        input = bsde.set_approximator_input()

        assert bsde.generate_backward_process().shape == (
            NUMBER_OF_PATHS,
            NUMBER_OF_TIMESTEPS,
            bsde.number_of_dimensions,
        )

        assert input.shape[-1] == bsde.y_approximator.gru.input_size
