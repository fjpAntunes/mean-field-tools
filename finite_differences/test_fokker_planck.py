...
def test_calculate_divergence_term_one_dim_linear():
    output = one_dim_fokker_planck._calculate_divergence_term(
        np.ones_like(ONE_DIM_DOMAIN)
    )
    np.testing.assert_array_almost_equal(
        output, np.ones_like(ONE_DIM_DOMAIN), decimal=2
    )


def test_calculate_divergence_term_one_dim_test_quadratic():
    output = one_dim_fokker_planck._calculate_divergence_term(ONE_DIM_DOMAIN)
    np.testing.assert_almost_equal(output, ONE_DIM_DOMAIN * 2, decimal=2)

def test_calculate_second_derivative_term_one_dim_linear():
    output = one_dim_fokker_planck._calculate_second_derivative_term(ONE_DIM_DOMAIN)
    np.testing.assert_array_almost_equal(output, np.zeros_like(ONE_DIM_DOMAIN), decimal = 2)

def test_calculate_second_derivative_term_one_dim_quadratic():
    output = one_dim_fokker_planck._calculate_second_derivative_term(ONE_DIM_DOMAIN ** 2)
    np.testing.assert_array_almost_equal(output[2:-2], 2*np.ones_like(ONE_DIM_DOMAIN)[2:-2], decimal = 2)
...
def test_calculate_divergence_term_two_dim_test_():
...
def test_calculate_second_derivative_term_two_dim_linear():
    ...
    np.testing.assert_array_almost_equal(output, np.zeros(shape=(2,2,1_000,1_000)), decimal = 2)

def test_calculate_second_derivative_term_two_dim_quadratic():
    output = two_dim_fokker_planck._calculate_second_derivative_term(X**2 + 2*X*Y + Y**2)
    np.testing.assert_array_almost_equal(output[:,:,2:-2, 2:-2], 2* np.ones(shape= (2,2,1_000,1_000))[:,:,2:-2,2:-2], decimal = 2)